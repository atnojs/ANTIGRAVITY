document.addEventListener('DOMContentLoaded', () => {
    ...
    // Helper: Convierte URL de imagen a etiqueta img si es necesario
    function processIconHtml(iconInput) {
        if (!iconInput) return '<i class="fa-solid fa-globe"></i>';

        const trimmed = iconInput.trim();

        // Si ya es HTML (contiene < ), lo dejamos como está
        if (trimmed.includes('<')) {
            return trimmed;
        }

        // Si parece una URL de imagen, la convertimos a <img>
        const isUrl = trimmed.match(/^(https?:\/\/|www\.|\/)/i) ||
            trimmed.match(/\.(png|jpg|jpeg|gif|svg|webp|ico)$/i);
        ...
        // Si no es nada reconocido, lo tratamos como clase de FontAwesome
        return trimmed;
    }
    ...
    function createTileCardElement(app, sourceArray) {
        ...
        // Icono de fondo decorativo (se agranda y rota en hover)
        const bgIconHtml = processIconHtml(app.iconHtml);

        card.innerHTML = `
            <div class="bg-icon">${bgIconHtml}</div>
            <div class="tile-head">
                <div class="tile-icon">${bgIconHtml}</div>
    ...
    function openFormForAdd(panelId = null) {
        ...
        formUrlInput.value = '';
        formDescInput.value = '';
        formIconInput.value = '<img src="/imagenes/logo_antonio.png" style="width:100%; height:100%; object-fit:contain;">';
    ...
    const searchInput = googleForm ? googleForm.querySelector('input[name="q"]') : null;
    const lensBtn = document.getElementById('lens-btn');
    const voiceBtn = document.getElementById('voice-btn');

    if (lensBtn) {
        lensBtn.addEventListener('click', () => {
            window.open('https://lens.google.com/upload?hl=es', '_blank', 'noopener');
        });
    }
