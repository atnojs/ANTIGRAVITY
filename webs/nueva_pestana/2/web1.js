document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadState() {
        try {
            ...
            // Reconstruct panels and standalone tools from the unified list
            loadedTools.forEach(item => {
                if (item.isPanelHeader) {
                    panels.push({
                        id: item.id,
                        title: item.name,
                        isOpen: false, // Always start closed as requested
                        tools: item.tools || []
                    });
                } else {
            ...
            appData = { panels, tools: standaloneTools };

        } catch (e) {
            console.error("Error al cargar el estado:", e);
            appData = { panels: [], tools: [] };
        }
        render();
    }
    ...
    async function saveState() {
        ...
        try {
            ...
            // Add panels as special items marked with `isPanelHeader`
            ...
            // Construct the old format payload that the server expects
            const payload = {
                password: '0',
            ...
            console.error("Error en la función saveState:", e);
    ...
    function createPanelElement(panel) {
        ...
        if (isEditable) {
            ...
            `;
            buttons.querySelector('[title="Añadir Enlace"]').addEventListener('click', (e) => { e.stopPropagation(); openFormForAdd(panel.id); });
            buttons.querySelector('[title="Editar Panel"]').addEventListener('click', (e) => { e.stopPropagation(); openPanelFormForEdit(panel); });
            buttons.querySelector('[title="Eliminar Panel"]').addEventListener('click', (e) => { e.stopPropagation(); deletePanel(panel.id); });
    ...
    function createTileCardElement(app, sourceArray) {
        ...
        if (isEditable) {
            ...
            editButtons.querySelector('.edit-btn').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); openFormForEdit(app, sourceArray); });
            editButtons.querySelector('.delete-btn').addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); deleteItem(app.id, sourceArray); });

            wrapper.addEventListener('dragstart', e => { e.stopPropagation(); isDragging = true; e.dataTransfer.setData('text/plain', app.id); setTimeout(() => wrapper.querySelector('.tile-card').classList.add('dragging'), 0); });
            wrapper.addEventListener('dragend', e => { e.stopPropagation(); isDragging = false; document.querySelectorAll('.dragging, .drop-target').forEach(el => el.classList.remove('dragging', 'drop-target')); });
            wrapper.addEventListener('dragover', e => { e.preventDefault(); e.stopPropagation(); if (document.querySelector('.dragging')) wrapper.querySelector('.tile-card').classList.add('drop-target'); });
            wrapper.addEventListener('dragleave', e => { e.stopPropagation(); wrapper.querySelector('.tile-card').classList.remove('drop-target'); });
    ...
    function openFormForEdit(app, sourceArray) {
        const panel = appData.panels.find(p => p.tools === sourceArray);
        formTitle.textContent = 'Editar Enlace'; 
        editingIdInput.value = app.id;
        editingPanelIdInput.value = panel ? panel.id : '';
    ...
    formSaveBtn.addEventListener('click', () => {
        ...
        const panelId = editingPanelIdInput.value;
        const newData = { id, name: formTitleInput.value.trim(), websiteUrl: formUrlInput.value.trim(), briefDescription: formDescInput.value.trim(), iconHtml: formIconInput.value.trim(), tags: [], timestampAdded: Date.now() };
        if (!newData.name || !newData.websiteUrl) { alert('El título y la URL son obligatorios.'); return; }
        
        let targetArray;
        if (panelId) {
            const panel = appData.panels.find(p => p.id === panelId);
            if(panel) targetArray = panel.tools;
        } else {
            targetArray = appData.tools;
        }

        if(!targetArray) {
            alert('Error: No se encontró el panel o contenedor.');
            return;
        }
        ...
        formModal.style.display = 'none'; render(); saveState();
    });
    ...
    panelFormSaveBtn.addEventListener('click', () => {
        ...
        if (existingPanel) {
            ...
            appData.panels.push({ id, title, isOpen: false, tools: [] });
        }
    ...
    function deletePanel(panelId) {
        if (confirm('¿Seguro que quieres eliminar este panel? Los enlaces dentro se moverán a la lista principal.')) {
            ...
            if (panelIndex > -1) {
                ...
                appData.tools.unshift(...(panel.tools || [])); // Move tools to main list
                render();
                saveState();
            }
        }
    ...
    // ===== GOOGLE SEARCH & LENS =====
    document.getElementById('lens-btn').addEventListener('click', () => window.open('https://lens.google.com/upload?hl=es', '_blank', 'noopener'));
    ...
    if (recog) {
        ...
        document.getElementById('voice-btn').addEventListener('click', () => recog.start());
