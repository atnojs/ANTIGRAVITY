...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function saveState() {
        ...
        localStorage.setItem('promptGalleryData', JSON.stringify(payload));

        // Intentar guardar en PHP
        try {
            await fetch('guardar_cambios.php', {
                method: 'POST',
    ...
    closeGeneratorBtn.onclick = () => { generatorModal.style.display = 'none'; };
            ...
            results.forEach((imageData, index) => {
                if (imageData) {
                    ...
                    resultItem.querySelector('.download-btn').onclick = (e) => {
                        ...
                        downloadImage(src, `generada_${idx}_${Date.now()}.png`);
                    };
        ...
        const requestBody = {
            contents: [{
                parts: [
                    ...
                    {
                        text: `Modifica esta imagen según las siguientes instrucciones. Mantén la identidad de las personas que aparecen. Instrucciones: ${prompt}`
                    }
                ]
            }],
            generationConfig: {
