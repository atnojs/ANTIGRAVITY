...
7.  **Auditoría Lighthouse** (`skills/auditoria_lighthouse`): Checklist de rendimiento, SEO y buenas prácticas antes de entregar.
8.  **Accesibilidad** (`skills/accesibilidad`): Verificación WCAG 2.1 A/AA (contraste, teclado, ARIA, semántica).
...
- **Modelo AI**: `gemini-3.1-flash-image-preview` (modelo por defecto obligatorio).
...
**Antes de entregar**, DEBO ejecutar las auditorías de calidad:

1.  **Auditoría Lighthouse**: Ejecutar el checklist completo de `skills/auditoria_lighthouse` (Rendimiento → SEO → Buenas Prácticas).
2.  **Accesibilidad**: Ejecutar el checklist completo de `skills/accesibilidad` (Semántica → Contraste → Teclado → Imágenes → Formularios → Contenido Dinámico).
3.  **Corregir** todos los problemas encontrados antes de entregar.
4.  **Incluir reporte** en la entrega final.
...
2.  **Sincronizar**: Si existe un remoto, ejecutar:
    - `git add .`
    - `git commit -m "Descripción de los cambios/creación"`
