document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. LOADING OVERLAY 3-RING (Estándar Antigravity)
    const loadingOverlay = document.getElementById('loadingOverlay');
    const loadingText = document.getElementById('loadingText');

    const showGlobalLoader = (text) => {
        if (loadingText) loadingText.textContent = text || 'IA Generando Obra Maestra...';
    ...
    const injectLightbox = () => {
        ...
        const closeLightbox = () => overlay.classList.remove('show');
    ...
    const handleRegenConfirm = async () => {
        ...
        try {
            const qSize = qualitySelect ? qualitySelect.value : '512';
            let finalPrompt = "";
            if (mode === 'bg') {
            ...
            const resultBase64 = await callImageAPI(baseImage, finalPrompt, qSize);
    ...
    // ==== History Manager ====
    const STORAGE_KEY = 'outfit_change_history';
    ...
    const loadHistoryFromManager = async () => {
        try {
    ...
    const saveToHistoryManager = async (item) => {
        try {
    ...
    const handleGenerateClick = async () => {
        ...
        const qualitySize = qualitySelect ? qualitySelect.value : '512';
        const currentStyle = {
        ...
        try {
            ...
            for (let i = 0; i < iterations; i++) {
                showGlobalLoader(`Generando imagen ${i + 1} de ${iterations} (${qualitySize}px)`);
                try {
                    if (i > 0) await new Promise(r => setTimeout(r, 2000));
                    const resultBase64 = await callImageAPI(originalImageBase64, prompt, qualitySize);
                    lastResult = resultBase64;
                    await addToHistory(resultBase64, currentStyle, prompt, qualitySize);
                } catch (innerError) {
            ...
            if (lastResult) {
                ...
                styleDescriptionOutput.classList.add('hidden');
                resetComparisonSlider();
                showToast(`¡${iterations} outfits generados! (${qualitySize}px)`);
            }
    ...
    const addToHistory = async (imageBase64, styleInfo, promptUsed, size) => {
        const historyItem = {
            image: imageBase64,
            style: styleInfo,
            prompt: promptUsed,
        ...
        await saveToHistoryManager(historyItem);
    ...
    const renderHistory = () => {
        ...
        // Asegurar que exista el botón de limpiar
        let clearBtn = historySection.querySelector('.clear-history-btn');
        if (!clearBtn) {
            const header = historySection.querySelector('h2');
            clearBtn = document.createElement('button');
            clearBtn.className = 'clear-history-btn';
            clearBtn.textContent = '🗑️ Limpiar Todo';
            clearBtn.style.cssText = 'font-size:0.75rem;color:#EF4444;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:6px;padding:4px 10px;cursor:pointer;transition:all 0.3s;';
            clearBtn.onmouseenter = () => { clearBtn.style.background = 'rgba(239,68,68,0.25)'; };
            clearBtn.onmouseleave = () => { clearBtn.style.background = 'rgba(239,68,68,0.1)'; };
            ...
            if (header) header.insertAdjacentElement('afterend', clearBtn);
        }
        ...
        history.forEach((item, index) => {
            ...
            actions.appendChild(createBtn('btn-sq-blue',
                ...
                async () => {
                    ...
                    try {
                        const res = await callImageAPI(originalImageBase64, item.prompt || constructPrompt(), qSize);
                        await addToHistory(res, item.style, item.prompt, qSize);
            ...
            actions.appendChild(createBtn('btn-sq-red',
                ...
                'Eliminar',
                () => {
                    if (item.id) HistoryManager.deleteItem(item.id);
    ...
    const callImageAPI = async (base64Image, prompt, targetSize = '512') => {
        const proxyUrl = 'proxy.php';
        // CAMBIO DE MODELO A GEMINI 3 PRO PREVIEW
        ...
        const resultDataUrl = await postProcessDataURL(`data:image/png;base64,${imageData}`);

        // Redimensionar al tamaño objetivo si es necesario
        const targetPx = parseInt(targetSize || '512', 10);
        if (targetPx > 0) {
            const img = await new Promise((res, rej) => {
                const im = new Image();
                im.onload = () => res(im);
                im.onerror = rej;
                im.src = resultDataUrl;
            });
            const maxDim = Math.max(img.naturalWidth, img.naturalHeight);
            if (maxDim > targetPx) {
                let w = img.naturalWidth, h = img.naturalHeight;
                if (w > h) {
                    h = Math.round((h * targetPx) / w);
                    w = targetPx;
                } else {
                    w = Math.round((w * targetPx) / h);
                    h = targetPx;
                }
