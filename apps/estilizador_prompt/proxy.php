...
function handleGenerate(array $request): void {
    ...
    if (strpos(strtolower((string)($request['model'] ?? '')), 'qwen') !== false) {
...
function handleQwenImage(array $request): void {
    ...
    // Imagen base/estilo de referencia (data URLs), como el resto de rutas.
    $images = [];
    ...
    if ($targetRatio === null) {
        $parts = explode(':', (string)($request['aspectRatio'] ?? '1:1'));
    ...
    if (is_file($cacheFile)) {
        ...
        if (is_array($cached) && !empty($cached['b64'])) {
            qwenImageResponse((string)$cached['b64'], $qwenRatio, $requested, $format);
        }
    }
    ...
    if ($err || ($code === 0 && $resp === '')) {
        respond(502, ['success' => false, 'error' => 'Error conexion OpenRouter: ' . ($err !== '' ? $err : 'sin respuesta del proveedor.')]);
    }
    if ($code >= 400) {
        $eb = json_decode($resp, true);
        $em = $eb['error']['message'] ?? $eb['error'] ?? ('HTTP ' . $code);
        if (is_array($em)) $em = json_encode($em);
        respond($code < 600 ? $code : 502, ['success' => false, 'error' => 'OpenRouter: ' . (string)$em]);
    }

    $jr = json_decode($resp, true);
    $imageData = is_array($jr) ? (string)($jr['data'][0]['b64_json'] ?? '') : '';
    if ($imageData === '') {
        respond(502, ['success' => false, 'error' => 'Qwen no devolvio imagen.']);
    }
    ...
    @unlink($lockFile);
    qwenImageResponse($imageData, $qwenRatio, $requested, $format);
}

// Respuesta con la MISMA forma JSON que el resto de rutas de imagen del
// proxy (bloque canonico compartido + metadatos de esta app).
function qwenImageResponse(string $imageData, string $qwenRatio, int $requested, string $format): void {
    ...
    respond(200, [
        ...
        'aspectRatio' => $qwenRatio,
        'requestedResolution' => $requested, 'effectiveResolution' => $requested,
        'resolutionAdjusted' => false,
        'requestedFormat' => $format, 'formatAdjusted' => $format !== 'png',
    ]);
}
