...
function App() {
    ...
    const renderCanvas = useCallback(() => {
        ...
        const onLoad = () => {
            ...
            // - Si el contenedor trae transparencia real (PNG sin fondo), usar SOLO alfa.
            // - Si no trae transparencia, asumir fondo blanco y usar brillo.
            ...
            // Detectar si hay transparencia REAL en la imagen original
            // Ojo: muchos PNG/JPG "con fondo" pueden venir con alfa = 255 en todos los píxeles.
            // Y algunos contenedores con fondo blanco pueden traer bordes anti-aliased con alfa ligeramente < 255.
            // Por eso usamos un conteo y un umbral por porcentaje.
            const ALPHA_SOLID = 250; // consideramos "opaco" si alfa >= 250
            ...
            const hasSignificantTransparency = transparencyRatio > 0.01; // >1% píxeles con alfa < 250

            const ALPHA_THRESHOLD = 10;      // 0-255: considera silueta si alfa > 10
            const WHITE_THRESHOLD = 250;     // 0-255: considera fondo si brillo > 250 (m�s estricto para no "comerse" blancos del sujeto)
            // Nota: si tu contenedor tiene un sujeto MUY blanco sobre fondo blanco, este enfoque siempre es dif�cil;
            // pero con un umbral m�s alto evitamos que el fondo "entre" como silueta por ruido.

            for (let i = 0; i < data.length; i += 4) {
                const r = data[i];
                const g = data[i + 1];
                const b = data[i + 2];
                const a = originalData[i + 3]; // alfa original

                const brightness = (r + g + b) / 3;

                let isInSilhouette;
                if (hasSignificantTransparency) {
                    // Caso PNG transparente: la silueta es donde hay alfa
                    isInSilhouette = a > ALPHA_THRESHOLD;
                } else {
                    // Caso fondo blanco: la silueta es donde NO es blanco
            ...
            // Escalar contenido para cubrir canvas
    ...
    }, [containerImage, contentImage, containerOpacity, contentOpacity, containerBW, contentBW, contentPos, applyGrayscale]);
    ...
    const handleMouseUp = (e) => {
        // Nota: onClick puede dispararse después de un drag; usamos refs para bloquearlo
                        ...
                        <div className="preview-canvas-container">
                            {canGenerate ? (
                                <canvas
                                    ...
                                    onClick={(e) => {
                                        // Evitar que un drag termine abriendo el visor
                    ...
                    >
                        {contentImage ? (
                            <>
                                ...
                                <button
                                    className="remove-btn"
                                    onClick={(e) => { e.stopPropagation(); setContentImage(null); setContentPos({ x: 0, y: 0 }); }}
                                >
                                    <X size={16} />
                                </button>
                            </>
