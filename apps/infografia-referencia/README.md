...
4. La IA generativa genera una nueva infografía usando la referencia y el JSON como guía.
5. El resultado se persiste en el historial del servidor y puede descargarse.

## Servidor

Subir esta carpeta a `public_html/apps/infografia-referencia/`. El servidor debe exponer las variables `O`/`OPENAI_API_KEY` (OpenAI Image) y `R` (OpenRouter) mediante `SetEnv` en el `.htaccess` raíz. Las claves nunca se incluyen en estos archivos.
