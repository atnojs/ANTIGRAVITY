...
app.post("/api/project/:id/package", (req, res) => {
  const { id } = req.params;
  const { deployType, subfolderPath } = req.body as { deployType: 'root' | 'subfolder', subfolderPath: string };
  const project = activeProjects.get(id);
  ...
  try {
    ...
    if (project.summary.technology === "React_Vite" || project.summary.technology === "Vue_Vite" || project.summary.technology === "NextJS") {
      const isSubfolder = deployType === "subfolder";
      const cleanSub = isSubfolder ? "/" + subfolderPath.replace(/^\/+|\/+$/g, "") : "";
