...
export default function App() {
  ...
  const [deployPathError, setDeployPathError] = useState('');
  ...
  const uploadFile = async (file: File) => {
    // Validar que haya ruta de despliegue
    const cleanPath = deployPath.trim().replace(/^\/+|\/+$/g, '');
    if (!cleanPath) {
      setDeployPathError('Define la ruta de despliegue antes de subir el ZIP');
      return;
    }
    setDeployPathError('');
    setUploading(true);
    ...
    formData.append("deployPath", cleanPath);
  ...
  const triggerCompile = async () => {
    ...
    try {
      const res = await fetch(`/api/project/${projectId}/compile`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ deployPath: deployPath.trim().replace(/^\/+|\/+$/g, '') })
      });
      if (res.ok) {
        setCompilation({
          status: "installing",
  ...
  const processPackage = async () => {
    ...
    try {
      const res = await fetch(`/api/project/${projectId}/package`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ deployType, subfolderPath, deployPath: deployPath.trim().replace(/^\/+|\/+$/g, '') })
      });
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? "bg-[#0f172a] text-slate-100" : "bg-slate-50 text-slate-900"}`}>
      ...
      <header className="border-b border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-[#111a2e]/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {projectId && (
              <button
                onClick={handleReset}
                className="text-xs font-semibold px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
                Reiniciar Flujo
              </button>
            )}
          ...
          {projectId && summary && (
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#1e293b] p-5 rounded-2xl shadow-xl shadow-slate-900/5 dark:shadow-slate-950/20 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-indigo-500 flex items-center justify-center text-white font-extrabold text-sm shadow-md shadow-indigo-500/20 uppercase">
                ...
                <div className="border-t border-slate-100 dark:border-slate-700/60 pt-3.5 space-y-2.5 text-xs">
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">Tecnología:</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 dark:text-slate-400">Archivos cargados:</span>
                    <span className="font-mono text-slate-700 dark:text-slate-300 font-medium">
                      {summary.fileCount}
                    </span>
                  </div>
                  ...
                  {summary.buildCommand && (
                    <div className="flex justify-between items-center">
                      <span className="text-slate-500 dark:text-slate-400">Comando Build:</span>
                      <span className="font-mono text-slate-700 dark:text-slate-300 text-[10px] bg-slate-50 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/80 px-1.5 py-0.5 rounded">
                        {summary.buildCommand}
                      </span>
                    </div>
                  )}
                  ...
                  {summary.outputFolder && (
                    <div className="flex justify-between items-center">
                      <span className="text-slate-500 dark:text-slate-400">Directorio salida:</span>
                      <span className="font-mono text-slate-700 dark:text-slate-300 text-[10px] bg-slate-50 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/80 px-1.5 py-0.5 rounded">
                        /{summary.outputFolder}
                      </span>
                    </div>
                ...
                <div className="border-t border-slate-100 dark:border-slate-700/60 pt-3.5 space-y-2">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-400 font-medium">Compatibilidad Hostinger:</span>
              ...
              <div className="border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#1e293b] p-5 rounded-2xl shadow-xl shadow-slate-900/5 dark:shadow-slate-950/20 space-y-3">
                ...
                <div className="rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-[#162032] p-2">
                ...
                <div className="space-y-6 py-6" id="wizard-step-upload">
                  <div className="text-center space-y-1">
                    <h2 className="text-xl md:text-2xl font-extrabold text-neutral-800 dark:text-neutral-100 flex items-center justify-center gap-2">
                      <Sparkles className="text-indigo-500 fill-indigo-500/10 animate-pulse" size={20} />
                  ...
                  {/* Ruta de despliegue en Hostinger */}
                  <div className="border border-indigo-200 dark:border-indigo-900/40 rounded-xl p-4 bg-indigo-500/5 space-y-2">
                    <label className="text-xs font-bold text-neutral-800 dark:text-neutral-200 flex items-center gap-1.5">
                      📁 Ruta de despliegue en Hostinger
                    </label>
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs text-neutral-500 dark:text-neutral-400 font-mono whitespace-nowrap">public_html/</span>
                      <input
                        type="text"
                        value={deployPath}
                        onChange={(e) => { setDeployPath(e.target.value); setDeployPathError(''); }}
                        placeholder="ej: webs/compilar"
                        className="flex-1 text-sm px-3 py-2.5 border-2 border-neutral-300 dark:border-neutral-700 rounded-lg bg-white dark:bg-neutral-900 text-neutral-800 dark:text-neutral-100 font-mono focus:outline-none focus:border-indigo-500 transition-colors"
                      />
                    </div>
                    <p className="text-[11px] text-neutral-400 leading-normal">
                      Indica la carpeta dentro de <code className="bg-neutral-100 dark:bg-neutral-800 px-1 py-0.5 rounded text-[10px]">public_html</code> donde se desplegará la app (ej: <code className="bg-neutral-100 dark:bg-neutral-800 px-1 py-0.5 rounded text-[10px]">Paginas/mi-app</code>). El compilador ajustará las rutas base automáticamente.
                    </p>
                    {deployPathError && (
                  ...
                  >
                    ...
                    {!uploading && (
                      <span className="text-[10px] font-semibold text-neutral-400 uppercase bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 px-2 py-0.5 rounded">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                    <div className="p-3 border border-neutral-200 dark:border-neutral-800/40 bg-neutral-50/50 dark:bg-neutral-900/10 rounded-xl space-y-1">
                  ...
                  {diagnosing ? (
                    <div className="text-center py-12 space-y-3">
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-neutral-400 block">Verificando enlazado sintáctico de imports y credenciales expuestas</span>
                      </div>
                    </div>
                  ) : (
                      ...
                      {issues.filter(i => i.canAutoFix && !i.fixed).length === 0 && (
                        <div className="flex justify-end pt-2">
                          <button
                            ...
                            className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs rounded-lg transition-colors cursor-pointer shadow-md shadow-indigo-600/10"
                          >
                ...
                <div className="space-y-6" id="wizard-step-adjustments">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-800">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs rounded-lg transition-colors cursor-pointer shadow-md shadow-indigo-600/10"
                    >
              ...
              {currentStep === 4 && (
                <div className="space-y-6" id="wizard-step-routing">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="border border-neutral-300 dark:border-neutral-800 rounded-xl overflow-hidden bg-neutral-950 shadow-inner">
                    ...
                    <p className="text-[11px] text-neutral-400 italic leading-normal">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-800">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all shadow-md shadow-indigo-600/10 cursor-pointer"
                      disabled={packaging}
                    >
                      {packaging ? (
                        <>
              ...
              {currentStep === 5 && (
                <div className="space-y-6" id="wizard-step-download">
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-neutral-200 dark:border-neutral-800">
                    ...
                    <button
                      onClick={handleReset}
                      className="text-xs px-3.5 py-1.5 hover:bg-neutral-100 dark:hover:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-md font-semibold text-neutral-600 dark:text-neutral-400 transition-colors"
                    >
      ...
      <footer className="h-10 bg-white dark:bg-[#1e293b] border-t border-slate-200 dark:border-slate-800/80 px-6 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 select-none shadow-md">
        ...
        <div className="flex items-center gap-4">
          <span className="bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 rounded text-slate-600 dark:text-slate-400 font-medium">Windows 11 Pro Optimized</span>
