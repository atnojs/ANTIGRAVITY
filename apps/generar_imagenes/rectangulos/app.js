...
(() => {
  ...
  const closestAR = (w, h) => {
    ...
    let diff = Infinity;
    VALID_AR.forEach((ar) => {
      const d = Math.abs(ratioValue(ar) - r);
      if (d < diff) {
        diff = d;
        best = ar;
      }
    ...
    return (
      <div className="mask-editor-overlay">
        <h3 style={{ color: "white", marginBottom: ".5rem" }}>
          Selecciona zonas con click y arrastre
        </h3>
  ...
  const App = () => {
    ...
    const [baseAR, setBaseAR] = useState("9:16");
    ...
    const onUploadBase = (e) => {
      if (e.target.files?.length > 0) {
        ...
        if (first) {
          const img = new Image();
          img.onload = () => {
            const ar = closestAR(img.naturalWidth, img.naturalHeight);
            setBaseAR(ar);
            setAspectRatio(ar);
          };
          img.src = first.url;
      ...
      try {
        ...
        if (res.type === "image") {
          const newImg = {
            ...
            promptLabel:
              effPrompt && effPrompt.trim() !== "" ? "Editado" : "Solo AR"
          };
          setImages((prev) => [newImg, ...prev]);

          if (baseImages.length > 0) {
            setAspectRatio(baseAR);
          }
      ...
      <div className="container">
        ...
        <div id="app-content">
          <div className="controls-column">
            ...
            <div className="actions" style={{ marginTop: "1.5rem" }}></div>
