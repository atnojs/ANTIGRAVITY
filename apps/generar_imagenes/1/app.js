            ...
            <div className="card glass-hover" ref={cardRef}>
                <div className={`ar-box ${maskObj ? 'has-mask-indicator' : ''}`}
                    ...
                    <div className="overlay-actions" onClick={e => e.stopPropagation()}>
                        ...
                        <button className="action-btn btn-3d" style={{ background: '#f59e0b' }} data-tooltip="Crear Máscara" onClick={() => onEditRequest(img, 'mask')}>
                            <i className="fa-solid fa-paintbrush"></i>
                        </button>
...
const App = () => {
        ...
        return (
            <div className="container">
                {isLoading && <div className="loading-overlay"><div className="spinner-triple"><div className="ring ring-1"></div><div className="ring ring-2"></div><div className="ring ring-3"></div></div><p className="loading-text">{loadingMsg}</p></div>}
                                    ...
                                    <div className="model-provider-column" key={group.name} style={{ minWidth: 0 }}>
                                        ...
                                        <div className="model-toggle-group" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '.35rem' }}>
                                            {group.models.map(([value, label]) => (
                                                <button key={value} type="button" className="btn btn-3d btn-sm" aria-pressed={provider === value}
                                                    onClick={() => setProvider(value)} style={{ padding: '.55rem .3rem', fontSize: '.72rem', background: provider === value ? 'linear-gradient(135deg, #22d3ee, #a78bfa)' : 'rgba(255,255,255,.06)', color: provider === value ? '#0c1445' : 'var(--text)' }}>
                                                    {label}
                                                </button>
                                            ))}
                                        </div>
                            ...
                            <div className="base-uploader-section">
                                <div className={`base-image-preview ${tempMaskData[baseImages[0].id] ? 'has-mask-indicator' : ''}`}>
                                    ...
                                    <div className="base-overlay-actions">
                                        <button className="base-action-btn btn-3d" title="Pintar área a editar" onClick={() => onBaseImageMask(baseImages[0])}>
                                            <i className="fa-solid fa-paintbrush"></i>
                                        </button>
