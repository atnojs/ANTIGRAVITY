...
if (is_array($agBody) && empty($agBody['action'])) {
    $agGenCfg = is_array($agBody['generationConfig'] ?? null) ? $agBody['generationConfig'] : [];
    $agHasImageParts = false;
    foreach ((array)($agBody['contents'][0]['parts'] ?? []) as $agPart) {
        if (!empty($agPart['inlineData']['data']) || !empty($agPart['inline_data']['data'])) { $agHasImageParts = true; break; }
    }
    $agModel = (string)($agBody['model'] ?? '');
    // Texto/visión (sin salida de imagen) → xiaomi/mimo-v2.6-pro vía OpenRouter.
    $agIsTextOnly = isset($agBody['contents']) && !$agHasImageParts
        && empty($agGenCfg['imageConfig'])
        && !in_array('IMAGE', array_map('strtoupper', (array)($agGenCfg['responseModalities'] ?? [])), true)
        && (strpos($agModel, 'xiaomi/') === 0 || stripos($agModel, 'image') === false);
    if ($agIsTextOnly) {
        [$httpcode, $response] = $mimoTextCall($agBody, $agGenCfg);
    ...
    if (strtolower($agModel) === 'qwen-pro') { handleQwenImage($agBody); exit; }
    ag_image_response($agBody, __DIR__);
}
...
// Detectar backend según el modelo (alias legado del modelo de texto: gemini-3.8-flash)
$modelParam = strtolower((string)($req['model'] ?? 'xiaomi/mimo-v2.6-pro'));
...
$hasImageParts = false;
foreach ((array)($req['contents'][0]['parts'] ?? []) as $part) {
    if (!empty($part['inlineData']['data']) || !empty($part['inline_data']['data'])) { $hasImageParts = true; break; }
}
$wantsImageOut = (stripos($reqModelName, 'image') !== false)
    ...
    || (isset($genCfg['imageConfig']) && is_array($genCfg['imageConfig']))
    || $hasImageParts;
