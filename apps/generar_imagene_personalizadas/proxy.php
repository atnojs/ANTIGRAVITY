...
function gpEnvKey(string $name): string
{
...
$openaiKey = gpEnvKey('OPENAI_API_KEY');
if ($openaiKey === '') $openaiKey = gpEnvKey('O');
...
$textModel = '';
if ($requested === 'xiaomi/mimo-v2.6-pro' || strpos($requested, 'xiaomi/') === 0) $textModel = 'xiaomi/mimo-v2.6-pro';
...
// ===== Utilidades OpenAI =====
function gpOpenAiSize(string $ratio): string
{
...
function gpSendGeminiStyleImage(string $binary, string $mimeType = 'image/png'): void
{
    echo json_encode([
        'candidates' => [[
...
if (isset($modelCatalog[$requested]) && $modelCatalog[$requested]['backend'] === 'openai') {
    ...
    $fields = ['model' => $selected['model'], 'prompt' => $openaiPrompt, 'quality' => $selected['quality'], 'size' => gpOpenAiSize($aspect)];
    ...
    gpSendGeminiStyleImage(base64_decode($imageData), $mimeOut);
}
...
// La ruta de GENERACIÓN de imágenes (gemini-flash / gemini-pro) queda intacta abajo.
if ($textModel !== '') {
    $genCfg = [];
...
if (isset($modelCatalog[$requested]) && $modelCatalog[$requested]['backend'] === 'qwen') {
    ...
    // a la proporción pedida (o a la imagen fuente si no se pidió ninguna).
    $qwenAllowed = [
    ...
    $targetRatio = 0.0;
    $qwenRefData = (string)($openaiImages[0]['data'] ?? '');
    $qwenRefMime = (string)($openaiImages[0]['mimeType'] ?? 'image/jpeg');
    if ($qwenRefData !== '' && strpos($qwenRefData, ',') !== false) {
        $qwenRefMatch = [];
        if (preg_match('#^data:(image/[a-z0-9.+-]+);base64,#i', $qwenRefData, $qwenRefMatch) === 1) {
            $qwenRefMime = strtolower($qwenRefMatch[1]);
            $qwenRefData = substr($qwenRefData, strpos($qwenRefData, ',') + 1);
        }
    }
    $qwenAspect = (string)($req['generationConfig']['imageConfig']['aspectRatio'] ?? $req['imageConfig']['aspectRatio'] ?? $req['aspectRatio'] ?? '');
    if ($qwenAspect !== '' && strpos($qwenAspect, ':') !== false) {
    ...
    $qwenCacheKey = hash('sha256', $modelCatalog[$requested]['model'] . '|' . $openaiPrompt . '|' . $qwenRefData . '|' . $qwenRatio);
    ...
    if (is_file($qwenCacheFile)) {
        ...
        if (is_array($qwenCached) && !empty($qwenCached['b64'])) {
            gpSendGeminiStyleImage(base64_decode((string)$qwenCached['b64']), 'image/png');
        }
    }
    ...
    gpSendGeminiStyleImage(base64_decode($imageData), 'image/png');
}
