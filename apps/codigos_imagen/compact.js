(function () {
    ...
    let temporaryModelImageData = '';
    let modelImageRemoved = false;
    let toastTimer = null;
    let root;
    let refs = {};
    ...
    function createInterface() {
        ...
        root.innerHTML = `
            <header class="compact-header">
                <button class="compact-title" id="compact-title" type="button" aria-label="Volver a los modelos">✦ GALERÍA DE PROMPTS PARA IMÁGENES PROFESIONALES</button>
            </header>
            ...
            <div class="modal-overlay" id="compact-model-modal" hidden>
                <section class="modal-card" role="dialog" aria-modal="true" aria-labelledby="compact-model-title">
                    ...
                    <form id="compact-model-form"><input id="compact-model-id" type="hidden"><label for="compact-model-name">Nombre del modelo</label><input id="compact-model-name" type="text" maxlength="40" placeholder="Ej. ChatGPT" required><div class="upload-row"><label class="upload-button" for="compact-model-file">↑ Seleccionar imagen</label><input id="compact-model-file" type="file" accept="image/jpeg,image/png,image/webp,image/gif"><span id="compact-model-file-name">Ninguna imagen seleccionada</span></div><div class="image-preview model-image-preview" id="compact-model-preview-wrap" hidden><img id="compact-model-preview" src="" alt="Vista previa de la imagen del modelo"><button id="compact-remove-model-preview" type="button">Quitar imagen</button></div><div class="modal-buttons"><button class="modal-button cancel" data-close="compact-model-modal" type="button">Cancelar</button><button class="modal-button save" type="submit">Guardar</button></div></form>
                </section>
            </div>
    ...
    function bindEvents() {
        ...
        refs.modelFile.addEventListener('change', previewModelFile);
        refs.removeModelPreview.addEventListener('click', clearModelPreview);
    ...
    function render() {
        ...
        refs.modelsView.hidden = inModel; refs.promptsView.hidden = !inModel; refs.editToolbar.hidden = !isEditable; refs.title.textContent = inModel ? model.name.toUpperCase() : '✦ GALERÍA DE PROMPTS PARA IMÁGENES PROFESIONALES';
    ...
    function renderModels() {
        ...
        state.models.forEach(function (model) {
            const card = document.createElement('button'); card.type = 'button'; card.className = 'model-folder'; card.innerHTML = '<span class="folder-image' + (model.image ? '' : ' is-empty') + '">' + (model.image ? '<img src="' + escapeAttr(model.image) + '" alt="Imagen de ' + escapeAttr(model.name) + '">' : '') + '</span><span class="folder-content"><span class="folder-name">' + escapeHtml(model.name) + '</span><span class="folder-count">' + model.items.length + ' ' + (model.items.length === 1 ? 'código' : 'códigos') + '</span></span><span class="folder-actions"><span class="small-action edit-model" title="Editar modelo">✎</span><span class="small-action danger delete-model" title="Eliminar modelo">×</span></span>';
            const folderImage = card.querySelector('.folder-image'); const imageElement = card.querySelector('img');
            if (imageElement) imageElement.addEventListener('error', function () { folderImage.classList.add('is-empty'); imageElement.remove(); });
    ...
    function createPromptCard(item, model) {
        ...
        card.innerHTML = '<div class="card-image' + (item.image ? '' : ' is-empty') + '">' + (item.image ? '<img src="' + escapeAttr(item.image) + '" alt="' + escapeAttr(item.title) + '">' : '') + '</div><div class="card-content"><h3 class="card-title">' + escapeHtml(item.title) + '</h3>' + (item.description ? '<p class="card-description">' + escapeHtml(item.description) + '</p>' : '') + '<button class="copy-button" type="button">⧉ &nbsp; Copiar código</button></div>' + (isEditable ? '<div class="card-actions"><button class="card-action edit-card" type="button" title="Editar código">✎</button><button class="card-action danger delete-card" type="button" title="Eliminar código">×</button></div>' : '');
    ...
    function openModelModal(model) { refs.modelForm.reset(); temporaryModelImageData = ''; modelImageRemoved = false; refs.modelId.value = model ? model.id : ''; refs.modelName.value = model ? model.name : ''; refs.modelFileName.textContent = 'Ninguna imagen seleccionada'; refs.modelTitle.textContent = model ? 'Editar modelo' : 'Añadir modelo'; if (model && model.image) showModelPreview(model.image); else clearModelPreview(); openModal('compact-model-modal'); setTimeout(function () { refs.modelName.focus(); }, 80); }
    function previewModelFile() { const file = refs.modelFile.files[0]; if (!file) return; if (file.size > 20 * 1024 * 1024) { showToast('La imagen no puede superar los 20 MB.', 'error'); refs.modelFile.value = ''; return; } refs.modelFileName.textContent = file.name; const reader = new FileReader(); reader.onload = function (event) { temporaryModelImageData = event.target.result; modelImageRemoved = false; showModelPreview(temporaryModelImageData); }; reader.readAsDataURL(file); }
    function showModelPreview(source) { refs.modelPreview.src = source; refs.modelPreviewWrap.hidden = false; }
    function clearModelPreview() { temporaryModelImageData = ''; modelImageRemoved = true; refs.modelPreview.src = ''; refs.modelPreviewWrap.hidden = true; refs.modelFile.value = ''; refs.modelFileName.textContent = 'Ninguna imagen seleccionada'; }
    async function uploadSelectedImage(file, fallback) { try { const data = new FormData(); data.append('image', file); const response = await fetch('upload.php', { method: 'POST', body: data }); const uploaded = await response.json(); return uploaded.success && uploaded.url ? uploaded.url : fallback; } catch (_) { return fallback; } }
    async function saveModel(event) { event.preventDefault(); const name = refs.modelName.value.trim(); if (!name) return; const id = refs.modelId.value; const model = id ? state.models.find(function (entry) { return entry.id === id; }) : null; let image = model && !modelImageRemoved ? model.image : ''; if (temporaryModelImageData && refs.modelFile.files[0]) image = await uploadSelectedImage(refs.modelFile.files[0], temporaryModelImageData); if (model) { model.name = name; model.image = image; } else { state.models.push({ id: 'model-' + Date.now(), name: name, image: image, items: [] }); } closeModal('compact-model-modal'); render(); const synced = await saveState(); showToast(synced ? (model ? 'Modelo actualizado.' : 'Modelo creado.') : 'No se pudo guardar en el servidor.', synced ? '' : 'error'); }
    async function deleteModel(model) { if (!confirm('¿Eliminar la carpeta “' + model.name + '” y sus códigos?')) return; state.models = state.models.filter(function (entry) { return entry.id !== model.id; }); if (activeModelId === model.id) activeModelId = ''; render(); const synced = await saveState(); showToast(synced ? 'Modelo eliminado.' : 'No se pudo guardar en el servidor.', synced ? '' : 'error'); }

    function openPromptModal(item, modelId) { const model = state.models.find(function (entry) { return entry.id === modelId; }); if (!model) return; refs.promptForm.reset(); temporaryImageData = ''; refs.promptId.value = item ? item.id : ''; refs.promptModelId.value = modelId; refs.promptName.value = item ? item.title : ''; refs.promptDescription.value = item ? item.description : ''; refs.promptText.value = item ? item.prompt : ''; refs.fileName.textContent = 'Ninguna imagen seleccionada'; refs.promptTitle.textContent = item ? 'Editar código' : 'Añadir código'; refs.promptKicker.textContent = model.name + ' · código de generación'; if (item && item.image) showPreview(item.image); else clearPreview(); openModal('compact-prompt-modal'); setTimeout(function () { refs.promptName.focus(); }, 80); }
    ...
    async function savePrompt(event) { event.preventDefault(); const model = state.models.find(function (entry) { return entry.id === refs.promptModelId.value; }); if (!model) return; const oldItem = refs.promptId.value ? model.items.find(function (entry) { return entry.id === refs.promptId.value; }) : null; let image = oldItem && !refs.previewWrap.hidden ? oldItem.image : ''; if (temporaryImageData && refs.promptFile.files[0]) { try { const data = new FormData(); data.append('image', refs.promptFile.files[0]); const response = await fetch('upload.php', { method: 'POST', body: data }); const uploaded = await response.json(); image = uploaded.success && uploaded.url ? uploaded.url : temporaryImageData; } catch (_) { image = temporaryImageData; } } const item = { id: refs.promptId.value || 'prompt-' + Date.now(), title: refs.promptName.value.trim(), description: refs.promptDescription.value.trim(), image: image, prompt: refs.promptText.value.trim() }; const index = oldItem ? model.items.indexOf(oldItem) : -1; if (index >= 0) model.items[index] = item; else model.items.push(item); closeModal('compact-prompt-modal'); render(); const synced = await saveState(); showToast(synced ? (oldItem ? 'Código actualizado.' : 'Código guardado.') : 'No se pudo guardar en el servidor.', synced ? '' : 'error'); }
    async function deletePrompt(item, model) { if (!confirm('¿Eliminar el código “' + item.title + '”?')) return; model.items = model.items.filter(function (entry) { return entry.id !== item.id; }); render(); const synced = await saveState(); showToast(synced ? 'Código eliminado.' : 'No se pudo guardar en el servidor.', synced ? '' : 'error'); }
