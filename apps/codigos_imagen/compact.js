(function () {
    ...
    function renderModels() {
        ...
        state.models.forEach(function (model) {
            const card = document.createElement('button'); card.type = 'button'; card.className = 'model-folder'; card.innerHTML = '<span class="folder-image' + (model.image ? '' : ' is-empty') + '">' + (model.image ? '<img src="' + escapeAttr(model.image) + '" alt="Imagen de ' + escapeAttr(model.name) + '">' : '') + '</span><span class="folder-name">' + escapeHtml(model.name) + '</span><span class="folder-count">' + model.items.length + ' ' + (model.items.length === 1 ? 'código' : 'códigos') + '</span><span class="folder-actions"><span class="small-action edit-model" title="Editar modelo">✎</span><span class="small-action danger delete-model" title="Eliminar modelo">×</span></span>';
    ...
    async function saveModel(event) { event.preventDefault(); const name = refs.modelName.value.trim(); if (!name) return; const id = refs.modelId.value; const model = id ? state.models.find(function (entry) { return entry.id === id; }) : null; let image = model && !modelImageRemoved ? model.image : ''; if (temporaryModelImageData && refs.modelFile.files[0]) image = await uploadSelectedImage(refs.modelFile.files[0], temporaryModelImageData); if (model) { model.name = name; model.image = image; } else { state.models.push({ id: 'model-' + Date.now(), name: name, image: image, items: [] }); } closeModal('compact-model-modal'); render(); saveState(); showToast(model ? 'Modelo actualizado.' : 'Modelo creado.'); }
