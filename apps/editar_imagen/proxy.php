...
// Modelo de TEXTO/VISIÓN (spec §6): xiaomi/mimo-v2.6-pro vía OpenRouter (respuesta forma Gemini).
// (Alias legacy 'gemini-3.8-flash'/'google/gemini-3.8-flash' conservado para flujos antiguos.)
$textModel = '';
if ($requested === 'xiaomi/mimo-v2.6-pro' || $requested === 'mimo' || $requested === 'google/gemini-3.8-flash' || $requested === 'gemini-3.8-flash') $textModel = 'xiaomi/mimo-v2.6-pro';
...
if (isset($modelCatalog[$requested]) && $modelCatalog[$requested]['backend'] === 'qwen') {
    ...
    $qwenModel = $modelCatalog[$requested]['model'];
    ...
    $qwenTarget = 0.0;
    $qwenReferences = [];
    foreach (array_slice($images, 0, 4) as $qwenImg) {
        $qwenRaw = (string)$qwenImg['data'];
        $qwenMime = (string)($qwenImg['mimeType'] ?? 'image/jpeg');
        if (preg_match('#^data:(image/[a-z0-9.+-]+);base64,#i', $qwenRaw, $qwenM) === 1) {
            $qwenMime = strtolower($qwenM[1]);
            $qwenRaw = substr($qwenRaw, strpos($qwenRaw, ',') + 1);
        }
        if ($qwenTarget === 0.0) {
            $qwenBin = base64_decode($qwenRaw, true);
        ...
        $qwenReferences[] = [
            'type' => 'image_url',
            'image_url' => ['url' => 'data:' . $qwenMime . ';base64,' . $qwenRaw],
        ];
    }
    if ($qwenTarget === 0.0) {
        $qwenAspect = (string)($req['generationConfig']['imageConfig']['aspectRatio'] ?? $req['aspectRatio'] ?? '1:1');
    ...
    if ($qwenReferences !== []) $qwenPayload['input_references'] = $qwenReferences;
    ...
    $qwenCacheKey = hash('sha256', $qwenModel . '|' . $prompt . '|' . json_encode($qwenReferences) . '|' . $qwenRatio);
    ...
    if (is_file($qwenCacheFile)) {
        ...
        if (is_array($qwenCached) && !empty($qwenCached['b64'])) {
            sendGeminiStyleImage(base64_decode((string)$qwenCached['b64']), 'image/png');
        }
    }
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER     => [
            ...
            'Authorization: Bearer ' . $qwenOrKey,
        ],
        CURLOPT_TIMEOUT        => 180,
        CURLOPT_CONNECTTIMEOUT => 20,
    ]);
    ...
    @unlink($qwenLockFile);
    sendGeminiStyleImage(base64_decode($qwenData), 'image/png');
}
...
if ($textModel !== '') {
    $genCfgMimo = [];
    if (isset($req['generationConfig']) && is_array($req['generationConfig'])) { $genCfgMimo = $req['generationConfig']; }
    elseif (isset($req['payload']['generationConfig']) && is_array($req['payload']['generationConfig'])) { $genCfgMimo = $req['payload']['generationConfig']; }
    [$httpcode, $response] = $mimoTextCall($req, $genCfgMimo);
