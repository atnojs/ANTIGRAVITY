/* app.js - Updated for Varied Backgrounds and Mobile Upload */
...
(function injectHoverStyles() {
    ...
    style.textContent = `
        ...
        .image-container.touched .image-action-btn {
            opacity: 1;
            visibility: visible;
...
// --- Configuración ---
const prompts = {
    'general': 'Wide shot of the exact same product from the provided image, integrated into a broad, realistic, and contextually varied environment (e.g., outdoor setting, modern studio, or natural scene, randomly chosen). Full scene, establishing shot, viewed straight from the front at eye level, maintain all details and branding. The background must be unique, photorealistic, and naturally complement the object.',
    'medium': 'Medium shot of the exact same product from the provided image, from the waist up or equivalent, viewed straight from the front at eye level, maintain all details and branding. Place the object in a varied, realistic environment (e.g., indoor setting, urban backdrop, or natural surface, randomly selected). The composition and lighting should appear natural and distinct from other shots.',
    'close-up': 'Extreme close-up shot of the exact same product from the provided image, tightly focusing on the main subject, viewed straight from the front, maintain all details and branding. The background should be a unique, contextually relevant, photorealistic setting with a soft, natural blur (bokeh effect) to highlight the object’s details, randomly chosen to differ from other shots.',
    'side': 'Side view of the exact same product from the provided image, profile shot, rotated 90 degrees to show the full side profile perpendicular to the original front view, maintain all details and branding. Generate a varied, photorealistic background (e.g., retail display, outdoor scene, or studio setting, randomly selected) that complements the main subject.',
    'top-down': 'True top-down (bird’s-eye) image of the exact same product from the reference, with the camera positioned directly above at a 90-degree angle, no tilt or perspective. Keep all product details, proportions, and branding identical. Lighting must be soft and even, with minimal shadows. The background should randomly vary between a realistic contextual surface (e.g., wooden table, marble counter, outdoor ground) or a clean neutral backdrop (white, light gray, or soft gradient), ensuring a unique and photorealistic appearance distinct from other shots.'
};
...
async function postProcessDataURL(dataURL, opts = {}) {
    ...
    for (let i = 0; i < d.length; i += 4) {
        ...
        b = mean + (b - mean) * (1 + k);
        if (Y > 0.6) {
            const wamt = cfg.warmHi * (Y - 0.6) / 0.4;
            r += 0.8 * wamt;
            b -= 0.8 * wamt;
        }
        d[i] = clamp(r * 255);
    ...
    if (cfg.unsharpAmt > 0) {
        ...
        bx.filter = `blur(${cfg.unsharpRadius}px)`;
        bx.drawImage(c, 0, 0);
        const src = x.getImageData(0, 0, w, h),
              blr = bx.getImageData(0, 0, w, h);
        const sd = src.data,
              bd = blr.data;
...
// Añadir soporte para eventos táctiles en móviles
...
async function generateImages(base64ImageData, mimeType, shotTypesToGenerate) {
    ...
    // Ajustar dinámicamente el número de columnas según el ancho de la pantalla
...
async function callImageAPI(base64ImageData, mimeType, shotType, cardId) {
    ...
    // Añadir un identificador único para forzar variación en el fondo
    const variationCue = `Variation seed: ${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
    const finalPrompt = composePrePrompt(`${prompt} ${variationCue}`, { integration });
    ...
    payload.generationConfig = Object.assign(
        { responseModalities: ["IMAGE"], temperature: 0.7 }, // Aumentar temperatura para más variación
